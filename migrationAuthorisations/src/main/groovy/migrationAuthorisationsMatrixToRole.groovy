...
if (strategy != null && strategy instanceof RoleBasedAuthorizationStrategy) {
    ...
    Jenkins.get().getItems(AbstractItem).each { item ->
        ...
        if (matcher.matches() && matcher.groupCount() == 1) {
            ...
            def roleviewaccess = checkViewRole(strategy, trigram, viewPermissionSet)
            ...
            strategy.assignRole(RoleBasedAuthorizationStrategy.VIEW, roleviewaccess, "PGG_APPLI_DEVOPS_DEV_${trigram.toUpperCase()}_ADVANCED_USER")
            strategy.assignRole(RoleBasedAuthorizationStrategy.VIEW, roleviewaccess, "PGG_APPLI_DEVOPS_DEV_${trigram.toUpperCase()}_USER")
            strategy.assignRole(RoleBasedAuthorizationStrategy.VIEW, roleviewaccess, "PGG_APPLI_DEVOPS_DEV_${trigram.toUpperCase()}_CONSULTATION")
            ...
            if( trigram.equals("xld")){
                ...
                strategy.assignRole(RoleBasedAuthorizationStrategy.VIEW, roleviewaccess, "authenticated")
            }
